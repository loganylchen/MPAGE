...
preprocess_intact_data <- function(input_file, 
  ...
  required_cols <- c("Alt..ID.s..interactor.A", "Alias.es..interactor.A", 
  ...
  processed_data <- df %>%
    ...
    dplyr::mutate(
      ...
      geneA_name = stringr::str_extract(Alias.es..interactor.A, 
                                       "uniprotkb:[A-Za-z0-9-]+\(gene name\)") %>%
                   stringr::str_remove("uniprotkb:") %>% 
                   stringr::str_remove("\(gene name\)") %>%
                   toupper(),
      geneB_name = stringr::str_extract(Alias.es..interactor.B, 
                                       "uniprotkb:[A-Za-z0-9-]+\(gene name\)") %>%
                   stringr::str_remove("uniprotkb:") %>% 
                   stringr::str_remove("\(gene name\)") %>%
                   toupper(),
      ...
      geneA_display = stringr::str_extract(Alias.es..interactor.A, 
                                          "display_short\):([^|]+)") %>%
                     stringr::str_remove("display_short\):"),
      geneB_display = stringr::str_extract(Alias.es..interactor.B, 
                                          "display_short\):([^|]+)") %>%
                     stringr::str_remove("display_short\):"),
    ...
    dplyr::filter(!is.na(geneA), !is.na(geneB), geneA != "", geneB != "",
  ...
  if (missing_proportion > max_missing_proportion) {
    warning(sprintf("High proportion of missing gene names: %.2f%%", 
